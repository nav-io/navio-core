...
BOOST_FIXTURE_TEST_CASE(tx_mempool_block_doublespend, Dersig100Setup)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        spends[i].vin[0].prevout.hash = m_coinbase_txns[0]->vout[0].GetHash();
...
BOOST_FIXTURE_TEST_CASE(checkinputs_test, Dersig100Setup)
{
    ...
    spend_tx.vin[0].prevout.hash = m_coinbase_txns[0]->vout[0].GetHash();
    ...
    // then test validity with P2SH.
    {
        ...
        invalid_under_p2sh_tx.vin[0].prevout.hash = spend_tx.vout[0].GetHash();
    ...
    // Test CHECKLOCKTIMEVERIFY
    {
        ...
        invalid_with_cltv_tx.vin[0].prevout.hash = spend_tx.vout[2].GetHash();
    ...
    // TEST CHECKSEQUENCEVERIFY
    {
        ...
        invalid_with_csv_tx.vin[0].prevout.hash = spend_tx.vout[3].GetHash();
    ...
    // for the same tx with a different witness.
    {
        ...
        valid_with_witness_tx.vin[0].prevout.hash = spend_tx.vout[1].GetHash();
...

    {
        ...
        tx.nVersion = 1;
        tx.vin.resize(2);
        tx.vin[0].prevout.hash = spend_tx.vout[0].GetHash();
        tx.vin[1].prevout.hash = spend_tx.vout[1].GetHash();
        tx.vout.resize(1);
