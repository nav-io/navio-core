...
BOOST_FIXTURE_TEST_CASE(template_defers_spend_of_inblock_staked_commitment, TestBLSCTChain100Setup)
{
    ...
    auto staked_a = blsct::CreateOutput(dest, 1000 * COIN, "stake-a", TokenId(), BlstScalar(uint256(uint64_t{0xa1})), blsct::CreateTransactionType::STAKED_COMMITMENT, 1000 * COIN);
    auto staked_b = blsct::CreateOutput(dest, 1500 * COIN, "stake-b", TokenId(), BlstScalar(uint256(uint64_t{0xb2})), blsct::CreateTransactionType::STAKED_COMMITMENT, 1500 * COIN);
    auto staked_c = blsct::CreateOutput(dest, 700 * COIN, "stake-c", TokenId(), BlstScalar(uint256(uint64_t{0xc3})), blsct::CreateTransactionType::STAKED_COMMITMENT, 700 * COIN);
    ...
    auto make_tx = [](const blsct::UnsignedOutput& staked, const COutPoint& prevout) {
        ...
        fee.predicate = blsct::PayFeePredicate(blsct::PublicKey(BlstG1Point::Rand())).GetVch();
    ...
    auto other = blsct::CreateOutput(dest, 300 * COIN, "stake-d", TokenId(), BlstScalar(uint256(uint64_t{0xd4})), blsct::CreateTransactionType::STAKED_COMMITMENT, 300 * COIN);
