...
void RangeProofToUniv(const bulletproofs_plus::RangeProof<Blst>& rp, UniValue& entry, const bool& extended)
{
...
void TxToUniv(const CTransaction& tx, const uint256& block_hash, UniValue& entry, bool include_hex, const CTxUndo* txundo, TxVerbosity verbosity, bool extendedRangeProof)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        if (tx.IsCoinBase()) {
            ...
            in.pushKV("outid", txin.prevout.hash.GetHex());
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++) {
        ...
        out.pushKV("hash", txout.GetHash().ToString());
