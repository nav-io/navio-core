...
{
    node::NodeContext& node = EnsureAnyNodeContext(request.context);
    if (!node.p2pmsg_user_inbox) throw JSONRPCError(RPC_MISC_ERROR, "p2pmsg user-message store disabled (-p2pmsgstoresize=0 or -p2pmsg=0)");
...
static RPCHelpMan sendp2pmsg()
{
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (!p2pmsg::IsValidTopic(frame.topic)) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "topic must be printable ASCII");
            }
...
static RPCHelpMan subscribep2pmsgtopic()
{
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (!p2pmsg::IsValidTopic(topic)) {
                // Match ingest: a non-ASCII subscription can never be matched
                // (ingest rejects such topics) and would taint listp2pmsgtopics.
                throw JSONRPCError(RPC_INVALID_PARAMETER, "topic must be printable ASCII");
            }
