...
void WalletInit::AddWalletOptions(ArgsManager& argsman) const
{
    ...
    argsman.AddArg("-consolidatestakedcommitments", strprintf("When staking, fold the wallet's existing staked commitments into the new commitment so the wallet holds a single consolidated stake (default: %u). Off by default: each stakelock is its own commitment, which keeps stakes additive and lets one wallet hold the >=2 distinct commitments a PoS membership ring needs. Delegated compounding and redelegation always fold.", blsct::DEFAULT_CONSOLIDATE_STAKED_COMMITMENTS), ArgsManager::ALLOW_ANY, OptionsCategory::WALLET);
