...
// is then a table-lookup + add per base, instead of the bucket tables blst's
// mulVec rebuilds on every call.
//
// The result of MSM() is bit-identical to BlstUtil::MSM over the same bases
...
public:
    ...
    size_t Bytes() const { return m_tbl.size() * sizeof(BlstG1Point::Underlying); }
...
private:
    ...
    BlstG1Point MulOne(size_t i, const BlstScalar& s) const;
