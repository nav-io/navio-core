...
//! Topics are routing keys that end up in RPC responses, logs and notifier
//! payloads verbatim; restrict them to printable ASCII so one inbound frame
//! cannot make every later listp2pmsgs response invalid UTF-8 (killing a
//! JSON client's drain loop) or forge log lines.
...
//! The node-parsed framing inside a USER_DATA payload: a routing topic plus
//! an opaque body. The topic is the ONLY part the node interprets (broadcast
//! subscriptions, store filtering); everything else an application needs
//! (sender identity, reply key, threading, content type) is its own framing
//! inside `body`.
struct UserMsgFrame {
    std::string topic;
    ...
    {
        // LIMITED_STRING: the plain std::string overload resizes to the
        // wire-declared compact size (up to MAX_SIZE = 128 MiB) BEFORE
        // reading -- on the decrypt path that lets a 5-byte broadcast
        // plaintext commit 128 MiB per in-flight message on every node.
        READWRITE(LIMITED_STRING(obj.topic, MAX_USER_MSG_TOPIC_BYTES), obj.body);
    }
};
