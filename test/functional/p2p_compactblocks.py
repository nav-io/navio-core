...
class CompactBlocksTest(BitcoinTestFramework):
    ...
    def make_utxos(self):
        ...
        tx = CTransaction()
        tx.vin.append(CTxIn(COutPoint(block.vtx[0].vout[0].hash()), b''))
        for _ in range(10):
        ...
        self.utxos.extend([[tx.vout[i].hash(), out_value] for i in range(10)])
    ...
    def build_block_with_transactions(self, node, utxo, num_transactions):
        ...
        for _ in range(num_transactions):
            tx = CTransaction()
            tx.vin.append(CTxIn(COutPoint(utxo[0]), b''))
            tx.vout.append(CTxOut(utxo[1] - 1000, CScript([OP_TRUE, OP_DROP] * 15 + [OP_TRUE])))
            tx.rehash()
            utxo = [tx.vout[0].hash(), tx.vout[0].nValue]
            block.vtx.append(tx)
    ...
    def test_getblocktxn_requests(self, test_node):
        ...
        self.utxos.append([block.vtx[-1].vout[0].hash(), block.vtx[-1].vout[0].nValue])
        comp_block = HeaderAndShortIDs()
