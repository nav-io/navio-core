...
INPUTS = [
    ...
    {'outid': '83a4f6a6b73660e13ee6cb3c6063fa3759c50c9b7521d0536022961898f4fb02',
...
class SignRawTransactionWithKeyTest(BitcoinTestFramework):
    ...
    def send_to_address(self, addr, amount):
        input = {"outid": self.nodes[0].getrawtransaction(self.nodes[0].getblock(self.block_hash[self.blk_idx])["tx"][0], 1)["vout"][0]["hash"]}
        output = {addr: amount}
        self.blk_idx += 1
    ...
    def verify_txn_with_witness_script(self, tx_type):
        ...
        outid = find_outid_for_address(self.nodes[0], txid, addr)
        self.generate(self.nodes[0], 1)
        # Now create and sign a transaction spending that output on node[0], which doesn't know the scripts or keys
        spending_tx = self.nodes[0].createrawtransaction([{'outid': outid}], {getnewdestination()[2]: Decimal("9.999")})
        spending_tx_signed = self.nodes[0].signrawtransactionwithkey(spending_tx, [embedded_privkey], [{'outid': outid, 'scriptPubKey': script_pub_key, 'redeemScript': redeem_script, 'witnessScript': witness_script, 'amount': 10}])
        self.assert_signing_completed_successfully(spending_tx_signed)
