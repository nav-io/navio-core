...
The NAVIO_BLSCT_FIXEDBASE window tables cannot be exercised in-process by the
...
2. restart with NAVIO_BLSCT_FIXEDBASE=1 (small window/prefix so table
...
FIXEDBASE_ENV = {
    "NAVIO_BLSCT_FIXEDBASE": "1",
    # w=4 over a 64-generator prefix keeps the tables at ~19 MiB and their
    # construction to a few hundred thousand point ops - cheap enough for CI
    # while still covering every single-output proof (mn = 64).
    "NAVIO_BLSCT_FIXEDBASE_WIN": "4",
    "NAVIO_BLSCT_FIXEDBASE_PREFIX": "64",
}
...
class BlsctFixedBaseVerifyTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Phase 1: build a BLSCT chain with the generic MSM")
        ...
        self.stop_node(0)
        self.start_node(0, env=FIXEDBASE_ENV)
