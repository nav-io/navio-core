...
class TxConflicts(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # | tx2 |  ----->  |                |         |               |
        #
        inputs_tx_AB_parent = [{"outid": self.nodes[0].getrawtransaction(txid_conflict_from_1,1)['vout'][output_A]['hash']}, {"outid": self.nodes[0].getrawtransaction(txid_conflict_from_2,1)['vout'][output_B]['hash']}]
        ...
        # | tx2 |  ----->  |     Tx_B_1     |
        #
        inputs_tx_A_1 = [{"outid": self.nodes[0].getrawtransaction(txid_conflict_from_1,1)['vout'][output_A]['hash']}]
        inputs_tx_B_1 = [{"outid": self.nodes[0].getrawtransaction(txid_conflict_from_2,1)['vout'][output_B]['hash']}]
        ...
        inputs_tx_C_child = [({"outid": self.nodes[0].getrawtransaction(txid_AB_parent,1)['vout'][output_c]['hash']})]
