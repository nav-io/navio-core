...
An envelope that ARRIVED as a flood is a different matter: its sender already
flooded it to every peer it has, so handing it back is a guaranteed replay
drop there and a wasted envelope here. The second half of this test pins that,
by watching what the dead end actually puts on the wire.
...
class P2PMsgStemDeadEndTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Now watch the wire instead of the leaves. With leaf_a gone, every
        # p2pmsg A sends goes to B and nowhere else, so bytes sent are a direct
        # measure of whether A reflected anything.
        self.log.info("A reflects a stem arrival, and only a stem arrival")
        leaf_a.peer_disconnect()
        leaf_a.wait_for_disconnect()
        self.wait_until(lambda: node_a.getp2pmsginfo()["leaf_peers"] == 0, timeout=20)

        def a_sent_p2pmsg():
            peers = node_a.getpeerinfo()
            assert_equal(len(peers), 1)  # B, and only B
            return peers[0]["bytessent_per_msg"].get("p2pmsg", 0)

        base = a_sent_p2pmsg()
        assert_equal(node_b.sendp2pping(node_a.getp2pmsginfo()["inbox_pubkey"], True), True)
        self.wait_until(lambda: a_sent_p2pmsg() > base, timeout=30)

        # A flood arrival must produce nothing. Fence on the leaf that is still
        # attached to B: once it has the envelope, B has finished flooding it,
        # so anything A was going to reflect has had its chance.
        after_stem = a_sent_p2pmsg()
        leaf_b_before = leaf_b.message_count["p2pmsg"]
        assert_equal(node_b.sendp2pping(node_a.getp2pmsginfo()["inbox_pubkey"], False), True)
        leaf_b.wait_until(lambda: leaf_b.message_count["p2pmsg"] > leaf_b_before, timeout=30)
        leaf_b.sync_with_ping()
        assert_equal(a_sent_p2pmsg(), after_stem)
